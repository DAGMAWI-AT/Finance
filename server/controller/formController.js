...
exports.adminCreateForm = async (req, res) => {
  ...
  try {
    ...
    const [staffCheck] = await pool.query(
      "SELECT role, name, registrationId FROM staff WHERE id = ?",
      [userId]
    );

    if (
      !staffCheck.length ||
      (staffCheck[0].role !== "admin" && staffCheck[0].role !== "sup_admin")
    ) {
    ...
    const notificationMessage = `Form "${form_name}" created by Finance Office (${staffCheck[0].name}). Please complete and submit it before the deadline.`;
    const notificationData = {
      notification_message: notificationMessage,
      registration_id: null,
      author: staffCheck[0].name,
      author_id: userId,
      application_id: 0,
    };

    await pool.query(`INSERT INTO ${notificationTable} SET ?`, [
      notificationData,
    ]);
...
exports.getFormById = async (req, res) => {
  try {
    ...
    form[0].user_submission =
      userSubmission.length > 0 ? userSubmission[0] : null;
...
const saveFile = (fileBuffer, formName, originalname) => {
  try {
    const { relativePath, absolutePath } = generateFilePath(
      formName,
      originalname
    );
...
exports.getAllSubmission = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Admin and Super Admin access required" });
    }

    const [submission] = await pool.execute(
      `SELECT * FROM applicationForm ORDER BY created_at DESC`
    );
...
exports.updateApplicationUpdatePermission = async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "update permission updated successfully",
    });
  } catch (error) {
...
exports.deleteApplicationForm = async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM comments WHERE report_id = ?", [id]);
...
exports.submitApplicationForm = async (req, res) => {
  ...
  try {
    ...
    const [form] = await pool.query("SELECT * FROM forms WHERE id = ?", [
      form_id,
    ]);
    ...
    const isUpdatePermissionClosed =
      hasExistingSubmission &&
      existingApplication[0].update_permission === "close";
    ...
    if (hasExistingSubmission) {
      ...
      // Insert notification
      const notificationMessage = `User ${row[0].name} has submitted the form "${form[0].form_name}". Please review it.`;
      const notificationData = {
        notification_message: notificationMessage,
      ...
      await connection.query(`INSERT INTO ${notificationTable} SET ?`, [
        notificationData,
      ]);
